...
**Dọn nợ kỹ thuật, đợt 1 — XONG 2026-09-25** (user chọn 3/4 mục khi được hỏi lại danh sách này):
...
5. **`window.confirm()` → modal riêng — CHƯA LÀM**, còn 25 chỗ ở 16 file. Khác `alert()`: mỗi `confirm()` đang GATE code chạy tiếp (`if (!window.confirm(...)) return;`), đổi sang modal Promise-based bắt hàm bao quanh phải `async`/`await` — rủi ro cao hơn, cần soát riêng từng chỗ, không mechanical như `alert()`. Để dịp sau (user đã đồng ý làm, nhưng dừng ở alert() trong đợt này để giữ mỗi đợt review được).

Còn lại chưa đụng (không nằm trong 4 mục user chọn đợt này): bundle **2.4 MB một mảnh**, không code-split; ~13 cụm fetch nổ cùng lúc lúc đăng nhập cho mọi role; `useNotifications` poll 45s không kiểm `document.visibilityState`; `App.tsx` 2600+ dòng / `MonthlyReportTabs.tsx` 2217 dòng; `/api/gemini/*` vẫn trả `isMock: true` kèm reply bịa khi thiếu `GEMINI_API_KEY`.
