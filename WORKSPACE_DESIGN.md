...
> **Trạng thái: GIAI ĐOẠN A + B1-B6 + C1-C4 ĐÃ CODE + VERIFY XONG.** Roadmap "6 module mới" gốc ở mục 6 đã hoàn thành. Giai đoạn C (4 module mới, mục 6.1) đã xong toàn bộ: **C1 — Onboarding Checklist theo Brand** (đã xóa 2026-08-09, xem "Đã xóa — Giai đoạn C1"), **C2 — GMV Target vs Actual theo Calendar**, **C3 — Scheme khuyến mãi tích hợp Calendar**, **C4 — Price List Import**. Toàn bộ roadmap workspace model (mục 6 + 6.1) đã hoàn thành. Sau đó thêm **Template hoá Campaign** (rà soát/nâng cấp module `ShiftScheduling.tsx`, xem mục dưới) — phiên sau cần user chỉ hướng đi tiếp theo (module mới ngoài roadmap này, hoặc rà soát/refactor phần đã có, vd rút gọn UI khối "Campaign Tháng" đang trùng lặp với `BrandCampaigns.tsx` — xem "Còn lại").
