...
> **Trạng thái: GIAI ĐOẠN A + B1-B6 + C1-C2 ĐÃ CODE + VERIFY XONG.** Roadmap "6 module mới" gốc ở mục 6 đã hoàn thành. Giai đoạn C (4 module mới, mục 6.1) đã xong **C1 — Onboarding Checklist theo Brand** và **C2 — GMV Target vs Actual theo Calendar**; còn 2 module C (Price List Import, Scheme tích hợp Calendar) chưa code, chờ user chọn thứ tự ở đầu phiên sau.
...
| **Scheme (khung giờ vàng/voucher) tích hợp vào Calendar** | Hiển thị **trực tiếp trên Lịch Vận Hành** (không phải tab riêng) — vì scheme hay thay đổi, cần dễ thấy/dễ tracking ngay tại nơi Ops đang nhìn lịch. **Việc còn mở, cần đề xuất UI/UX ở phiên implement** trước khi code: overlay lên ô ngày trong `LiveCalendar.tsx`/`BrandCalendar.tsx`, hay 1 hàng riêng phía trên lịch, hay badge/tooltip theo từng session. | Đây là điểm cần bàn kỹ nhất trong 4 module — không code trước khi thống nhất UX. Chưa code. |
