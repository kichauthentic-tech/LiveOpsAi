...
- **`brand_dataraw_imports` chỉ được 1 batch/`brand_id`+`report_type`+tháng của `period_start`** (unique index `idx_brand_dataraw_imports_brand_type_month`, migration 0077 — khớp `monthKey()`/`findExistingImportForMonth()` trong `lib/db/brandDataRaw.ts`). ⚠️ **Cần xác nhận lại trên Supabase thật**: bản 0077 commit ngày 2026-09-17 viết câu tạo index không ép kiểu nên **không chạy được trên bất kỳ Postgres nào** (xem quy ước `date_trunc` ở trên) — nghĩa là hoặc index trên DB thật có định nghĩa khác bản trong repo, hoặc nó chưa từng tồn tại và luồng chống-trùng-batch đang chỉ có ở tầng app. File đã sửa 2026-09-18 và chạy sạch cả chuỗi 0001→0082 trên Postgres cô lập; chạy lại 0077 trên Supabase (`if not exists` nên vô hại nếu index đã có) sẽ trả lời dứt điểm. Mọi ghi mới vào bảng này phải qua `createOrReplaceDataRawImport()`, không insert thẳng — hàm đã dịch lỗi `23505` (trùng batch do race/double-submit) sang message tiếng Việt, đừng bọc thêm lần nữa ở component.
...
1. ~~Không có notification nào xuyên suốt cả 3 bước~~ — **đã fix 2026-09-18** (chờ chạy migration 0083), xem mục 8 lộ trình bên dưới.
