// xlsx (~1 MB mã nguồn, lớn nhất bundle) chỉ cần lúc bấm "Tải Excel" → tải động khi gọi, không nằm trong chunk
// của màn (audit UX 2026-09-26, P2). Vì vậy 2 hàm dưới là async; lỗi (mất mạng lúc tải thư viện) do caller báo.
...
export function downloadRowsAsXlsx(sheetName: string, rows: Record<string, string | number>[], filename: string): Promise<void> {
  return downloadSheetsAsXlsx([{ name: sheetName, rows }], filename);
}
...
export async function downloadSheetsAsXlsx(sheets: { name: string; rows: Record<string, string | number>[] }[], filename: string): Promise<void> {
