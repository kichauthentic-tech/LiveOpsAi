...
import { LiveSession, MinuteMetric, ProductSKU } from "../types";
...
export const LiveSessionHub: React.FC<LiveSessionHubProps> = ({
  ...
  const [sessBrandName, setSessBrandName] = useState("Cocoon Vietnam");
  const [sessShopHandle, setSessShopHandle] = useState("@cocoon.vietnam.official");
  const [sessStudioName, setSessStudioName] = useState("Studio A - Flagship");
  const [sessHostName, setSessHostName] = useState("Hà An");
  ...
  const openAddSessionModal = () => {
    ...
    setSessBrandName("Cocoon Vietnam");
    setSessShopHandle("@cocoon.vietnam.official");
    setSessStudioName("Studio A - Flagship");
    setSessHostName("Hà An");
  ...
  const openEditSessionModal = (s: LiveSession) => {
    ...
    setSessBrandName(s.brandName);
    setSessShopHandle(s.shopTikTokHandle);
    setSessStudioName(s.studioName);
    setSessHostName(s.hostName);
  ...
  const handleSaveSession = (e: React.FormEvent) => {
    ...
    const sessionPayload: LiveSession = {
      ...
      title: sessTitle || `Live ${sessBrandName}`,
      brandId: editingSession?.brandId || "brand-1",
      brandName: sessBrandName,
      shopTikTokHandle: sessShopHandle,
      studioId: editingSession?.studioId || "studio-1",
      studioName: sessStudioName,
      hostId: editingSession?.hostId || "talent-1",
            ...
            <form onSubmit={handleSaveSession} className="p-6 space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    required
                    value={sessBrandName}
                    onChange={(e) => setSessBrandName(e.target.value)}
                    placeholder="VD: Cocoon Vietnam"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={sessStudioName}
                    onChange={(e) => setSessStudioName(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={sessHostName}
                    onChange={(e) => setSessHostName(e.target.value)}
                    placeholder="Hà An"
                    className="w-full p-2.5 border border-slate-200 rounded-xl font-semibold text-slate-800"
                  />
                </div>
