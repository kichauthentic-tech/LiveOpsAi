...
export const Dashboards: React.FC<DashboardsProps> = ({
  ...
  const subTabHeader = (
    <div className="flex flex-wrap items-center justify-between bg-[var(--surface)]/90 p-2 rounded-2xl border border-[var(--border)] shadow-md mb-4 gap-2">
      ...
      <div className="text-[11px] text-[var(--text-muted)] font-medium hidden sm:block pr-3">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-2">
            ...
            <div className="text-2xl font-black text-[var(--text)]">{totalActualGmv.toLocaleString()} đ</div>
          ...
          <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-2">
            ...
            <div className="text-2xl font-black text-[var(--text)]">{totalSessionsCount} Phiên</div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {/* GMV Trend Chart */}
          <div className="lg:col-span-2 bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="font-bold text-[var(--text)] text-base">Xu Hướng Doanh Thu GMV Theo Tuần (Triệu VNĐ)</h3>
              </div>
              <span className="text-xs font-bold bg-[var(--surface-elevated)] text-[var(--text-muted)] px-2.5 py-1 rounded-lg">Tuần Hiện Tại / 2026</span>
            </div>
          ...
          <div className="bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-[var(--text)] text-base flex items-center gap-1.5">
                ...
                talents.map((t, i) => (
                  <div key={t.id} className="flex items-center justify-between p-3 rounded-xl bg-[var(--surface-elevated)]/40 border border-[var(--border)]">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-[var(--text-muted)]">CVR: {t.cvrAvg}%</div>
                    </div>
                  </div>
                ))
              )}
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {studios.map((s) => (
            <div key={s.id} className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-xs text-[var(--text-muted)] font-medium">{s.roomNumber}</span>
                  <h3 className="font-bold text-[var(--text)] text-base">{s.name}</h3>
                </div>
              ...
              <p className="text-xs text-[var(--text-muted)] bg-[var(--surface-elevated)]/40 p-2.5 rounded-xl border border-[var(--border)]">{s.theme}</p>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
            <span className="text-xs text-[var(--text-muted)] font-medium block">Tổng GMV Đã Tích Lũy</span>
            <div className="text-2xl font-black text-[var(--text)]">1,250,000,000 đ</div>
            <span className="text-[10px] text-emerald-600 font-semibold">Vượt 115% KPI Cam Kết</span>
          </div>
          <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
            <span className="text-xs text-[var(--text-muted)] font-medium block">Tổng Số Đơn Hàng Thành Công</span>
            <div className="text-2xl font-black text-[var(--text)]">4,320 Đơn</div>
            <span className="text-[10px] text-[var(--text-muted)]">Tỷ lệ hủy/hoàn: 2.1% (Mức an toàn)</span>
          </div>
          <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
            <span className="text-xs text-[var(--text-muted)] font-medium block">ROI Trung Bình Trận Live</span>
            <div className="text-2xl font-black text-emerald-600">6.8x</div>
            <span className="text-[10px] text-[var(--text-muted)]">Doanh thu / Chi phí Agency</span>
          </div>
        </div>

        <div className="bg-[var(--surface)] p-6 rounded-2xl border border-[var(--border)] shadow-sm space-y-4">
          <h3 className="font-bold text-[var(--text)] text-base">Lịch Sử Phiên Live & Video Replay</h3>
          <div className="space-y-3">
            {sessions.map((s) => (
              <div key={s.id} className="p-4 rounded-xl border border-[var(--border)] flex flex-wrap justify-between items-center gap-4 hover:border-emerald-400 transition-all">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[var(--text)] text-sm">{s.title}</span>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
          <span className="text-xs text-[var(--text-muted)] font-medium block">Thu Nhập Commission Dự Kiến</span>
          <div className="text-2xl font-black text-emerald-600">32,500,000 đ</div>
          <span className="text-[10px] text-[var(--text-muted)]">Tháng 07/2026 (12 phiên live)</span>
        </div>
        <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
          <span className="text-xs text-[var(--text-muted)] font-medium block">Tỷ Lệ Chuyển Đổi Trung Bình (CVR)</span>
        ...
        <div className="bg-[var(--surface)] p-5 rounded-2xl border border-[var(--border)] shadow-sm space-y-1">
          <span className="text-xs text-[var(--text-muted)] font-medium block">Thời Gian Giữ Chân Khán Giả</span>
          <div className="text-2xl font-black text-indigo-600">3 Phút 04 Giây</div>
          <span className="text-[10px] text-[var(--text-muted)]">Giữ mắt xem ổn định top 1 agency</span>
        </div>
      </div>
